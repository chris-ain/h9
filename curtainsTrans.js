...
function curtainsTransFunc(smoothScroll){
  ...
  curtainsTrans.onRender(() => {
    ...
    const delta = curtainsTrans.getScrollDeltas();
...
if(!useNativeScroll) {
    ...
    var planeElements = document.getElementsByClassName("plane_trans");
    ...
    for(let i = 0; i < planeElements.length; i++) {
        const plane = new Plane(curtainsTrans, planeElements[i], params);
        plane.onError(() => {
            plane.remove();
        });
        planes.push(plane);
    ...
    function handlePlanes(index) {
        ...
        plane.onReady(() => {
            ...
            if(plane.userData.isFullscreen) {
                ...
                const curtainBoundingRect = curtainsTrans.getBoundingRect();
                ...
                plane.setRelativeTranslation(new Vec3(
                    -1 * planeBoundingRect.left / curtainsTrans.pixelRatio,
                    -1 * planeBoundingRect.top / curtainsTrans.pixelRatio,
                    0
                ));
            }
    ...
    galleryState.closeButtonEl.addEventListener("click", () => {
        const fullScreenPlane = curtainsTrans.planes.find(plane => plane.userData.isFullscreen);
        ...
        if(fullScreenPlane && galleryState.fullscreenThumb) {
            ...
            const allOtherPlanes = curtainsTrans.planes.filter(el => el.uuid !== fullScreenPlane.uuid && el.type !== "PingPongPlane");
    ...
    function onPlaneClick(event, plane) {
        ...
        if(!galleryState.fullscreenThumb) {
            ...
            galleryState.openTween = gsap.to(animation, 2, {
                ...
                translationX: -1 * planeBoundingRect.left / curtainsTrans.pixelRatio,
                translationY: -1 * planeBoundingRect.top / curtainsTrans.pixelRatio,
                transition: 1,
                textureScale: 1,
                mouseX: 0,
                mouseY: 0,
                ...
                onComplete: function() {
                    ...
                    const nonClickedPlanes = curtainsTrans.planes.filter(el => el.uuid !== plane.uuid && el.type !== "PingPongPlane");
    ...
    const bbox = curtainsTrans.getBoundingRect();
    ...
    // our ping pong plane
    const flowMap = new PingPongPlane(curtainsTrans, curtainsTrans.container, flowMapParams);
    ...
    flowMap.onRender(() => {
        ...
        flowMap.uniforms.velocity.value = new Vec2(curtainsTrans.lerp(velocity.x, 0, 0.1), curtainsTrans.lerp(velocity.y, 0, 0.1));
    }).onAfterResize(() => {
    ...
    const shaderPass = new ShaderPass(curtainsTrans, passParams);
    ...
    flowTexture.onSourceUploaded(() => {
        const fxaaPass = new FXAAPass(curtainsTrans);
    });
...
function destroyPlaneTrans() {
  ...
  export { curtainsTrans, planesTrans, curtainsTransFunc, destroyPlaneTrans };